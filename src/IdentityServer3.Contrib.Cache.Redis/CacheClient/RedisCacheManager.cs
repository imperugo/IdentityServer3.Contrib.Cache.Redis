...
namespace IdentityServer3.Contrib.Cache.Redis.CacheClient
{
    ...
    public class RedisCacheManager : ICacheManager
    {
        ...
        private readonly static ConcurrentDictionary<string, object> lockerObject = new ConcurrentDictionary<string, object>();
        private static bool isCachingEnabled = false;
        ...
        {
            return GetAndFill<T>(cacheKey, getData, TimeSpan.FromMinutes(30));
        }
        ...
        {
            return await GetAndFillAsync<T>(cacheKey, getData, TimeSpan.FromMinutes(30));
        }
        ...
        {
            if (!isCachingEnabled) { return getData(); }
            try
            {
                T data = Get<T>(cacheKey);
                if (data == null)
                {
                    object cacheLogObject = lockerObject.GetOrAdd(cacheKey, new object());
                    lock (cacheLogObject)
                    {
                        data = Get<T>(cacheKey);
                        if (data == null)
        ...
        public async Task<T> GetAndFillAsync<T>(string cacheKey, Func<Task<T>> getData, TimeSpan expiration)
        {
            ...
            if (!isCachingEnabled) { return await getData(); }
            try
            {
                T data = await GetAsync<T>(cacheKey);
                if (data == null)
                {
                    data = await getData();
                    Add(cacheKey, data, expiration);
        ...
        {
            return m_cacheClient.Add<T>(GenerateKey(key), value);
        }
        ...
        {
            return await m_cacheClient.AddAsync<T>(GenerateKey(key), value);
        ...
        {
            return m_cacheClient.Add<T>(GenerateKey(key), value, expiresAt);
        }
        ...
        {
            return await m_cacheClient.AddAsync<T>(GenerateKey(key), value, expiresAt);
        ...
        {
            IList<Tuple<string, T>> newItems = items.Select(item => new Tuple<string, T>(GenerateKey(item.Item1), item.Item2)).ToList();
        ...
        {
            return m_cacheClient.SetAdd<T>(GenerateKey(key), item);
        ...
        {
            return await m_cacheClient.SetAddAsync<T>(GenerateKey(key), item);
        }
        ...
        {
            return m_cacheClient.Publish<T>(channel, message, flags);
        }
        ...
        {
            return await m_cacheClient.PublishAsync<T>(channel, message, flags);
        ...
        {
            m_cacheClient.Subscribe<T>(channel, handler, flags);
        }
        ...
        {
            await m_cacheClient.SubscribeAsync<T>(channel, handler, flags);
        }
        ...
        {
            m_cacheClient.Unsubscribe<T>(channel, handler, flags);
        ...
        public async Task UnsubscribeAsync<T>(RedisChannel channel, Func<T, Task> handler,
            CommandFlags flags = CommandFlags.None)
        {
            await m_cacheClient.UnsubscribeAsync<T>(channel, handler, flags);
        }
        ...
        {
            return m_cacheClient.ListAddToLeft<T>(GenerateKey(key), item);
        ...
        {
            return await m_cacheClient.ListAddToLeftAsync<T>(GenerateKey(key), item);
        }
        ...
        {
            m_cacheClient.HashSet(hashKey, values.Select(v => new { key = GenerateKey(v.Key), value = v.Value }).ToDictionary(c => c.key, c => c.value), commandFlags);
        }
        ...
        {
            return await m_cacheClient.HashDeleteAsync(hashKey, keys.Select(key => GenerateKey(key)).AsEnumerable(), commandFlags);
        }
        ...
        {
            return await m_cacheClient.HashGetAsync<T>(hashKey, keys.Select(key => GenerateKey(key)).AsEnumerable(), commandFlags);
        }
        ...
        public async Task HashSetAsync<T>(string hashKey, IDictionary<string, T> values, CommandFlags commandFlags = CommandFlags.None)
        {
            ...
            await m_cacheClient.HashSetAsync(hashKey, values.Select(v => new { key = GenerateKey(v.Key), value = v.Value }).ToDictionary(c => c.key, c => c.value), commandFlags);
        }
